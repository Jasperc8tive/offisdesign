...
async function seedSuperAdmin() {
  ...
  const envEmail = process.env.SEED_SUPER_ADMIN_EMAIL;
  const envPassword = process.env.SEED_SUPER_ADMIN_PASSWORD;

  if (isProd && (!envEmail || !envPassword)) {
    throw new Error(
      'Refusing to seed in production without SEED_SUPER_ADMIN_EMAIL and SEED_SUPER_ADMIN_PASSWORD set.',
    );
  }
  ...
  const email = envEmail ?? 'admin@offisdesign.local';
  const password = envPassword ?? 'change-me-immediately';
